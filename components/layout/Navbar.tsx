      ...
      >
        <div className="section-container py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            <div className={`flex items-center gap-2 pr-3 border-r ${isScrolled ? 'border-forest/20' : 'border-cream/20'}`}>
              {Object.entries(CONTACT_INFO.socialMedia).map(([platform, url]) => (
                <Link
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 ${
                    isScrolled
                      ? 'text-charcoal/60 hover:text-gold bg-transparent hover:bg-gold/10'
                      : 'text-cream/70 hover:text-cream bg-white/5 hover:bg-white/10'
                  }`}
                >
                  {platform === 'youtube' && (
